...
const {
    json
} = require("express"); // acquire json for value get
...
app.use(express.json()); // it is used a bit for get value 



app.use(express.urlencoded({
    extended: false
})); // it is used with json to get values 
...
app.get("/", (req, resp) => { // app.get acess route and callback funtion 
    resp.render("home")
});

app.get("/Home", (req, resp) => { // app.get acess route and callback funtion 
    resp.render("home")
});


app.get("/Books", (req, resp) => { // app.get acess route and callback funtion 
    resp.send("work complete soon")
});

app.get("/category", (req, resp) => { // app.get acess route and callback funtion 
    resp.send("work complete soon")
});
...
app.post("/register", async (req, resp) => {
    ...
    try {
        ...
        if (Password === cpassword) {
            const registerStudent = new Register({ //take from above line 11
...
app.listen(port, () => { // it used callback function to check 
